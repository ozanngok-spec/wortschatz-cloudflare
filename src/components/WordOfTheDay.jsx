...
export function WordOfTheDay({ wotd, loading, alreadyAdded, onAdd, adding, onRefresh, speechLang = "de-DE", uiLang }) {
  const th = useTheme();
  ...
  const today = new Date().toLocaleDateString(uiLang ? speechLang : "en-GB", { weekday:"long", day:"numeric", month:"long" });
  ...
  if (loading) return (
    <div style={{ background:th.accentBg, border:`1.5px solid ${borderColor}`, borderRadius:12, padding:"14px 18px", marginBottom:8 }}>
      <div style={{ fontSize:10, color:th.accent, letterSpacing:"0.12em", textTransform:"uppercase", fontWeight:600 }}>✦ {s("wotdTitle", "Word of the Day")}</div>
      <div style={{ color:th.textFaint, fontSize:13, marginTop:6 }}>Loading…</div>
    </div>
  );
    ...
    <div style={{ background:th.accentBg, border:`1.5px solid ${borderColor}`, borderRadius:12, marginBottom:8, overflow:"hidden", boxShadow: th.isDark ? "none" : "0 1px 4px rgba(67,56,202,0.08)" }}>
      <div onClick={toggleExpanded} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:th.isMobile?"10px 14px":"13px 18px", cursor:"pointer" }}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          <span style={{ fontSize:10, color:th.accent, letterSpacing:"0.12em", textTransform:"uppercase", fontWeight:700 }}>✦ {s("wotdTitle", "Word of the Day")}</span>
      ...
      {expanded && (
        <div style={{ borderTop:`1px solid ${borderColor}`, padding:th.isMobile?"12px 14px 16px":"16px 18px 20px" }}>
          <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap", marginBottom:4 }}>
            ...
            <SpeakBtn text={wotd.word} size={14} lang={speechLang} />
          ...
          <div style={{ fontSize:10, color:th.textFaint, letterSpacing:"0.12em", textTransform:"uppercase", fontWeight:600, marginBottom:10 }}>{s("exampleSentences", "Example sentences")}</div>
            ...
            {(wotd.sentences||[]).map((s,i) => (
              <div key={i} style={{ borderLeft:`2px solid ${borderColor}`, paddingLeft:12 }}>
                <div style={{ fontSize:13, color:th.text, lineHeight:1.65, marginBottom:2, display:"flex", alignItems:"center", gap:6, flexWrap:"wrap" }}>
                  <span>
                    {(s.target??s.german).split(new RegExp(`(${wotd.word})`,'gi')).map((part,j) =>
                      part.toLowerCase()===wotd.word.toLowerCase() ? <span key={j} style={{ color:th.accent }}>{part}</span> : part
                    )}
                  </span>
                  <SpeakBtn text={s.target??s.german} size={11} lang={speechLang} />
                </div>
          ...
          <div style={{ display:"flex", gap:10, flexWrap:"wrap", alignItems:"center" }}>
            <button onClick={onAdd} disabled={alreadyAdded || adding}
              ...
              {alreadyAdded ? s("alreadyInVocab", "✓ Already in your vocabulary") : adding ? "…" : s("addToVocab", "+ Add to vocabulary")}
            </button>
