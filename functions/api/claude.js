export async function onRequest({ request, env }) {
  ...
  try {
    ...
    if (!response.ok) {
      return new Response(JSON.stringify({ error: `Anthropic API error ${response.status}`, detail: data }), {
        status: response.status, headers: { "Content-Type": "application/json" }
      });
    }

    if (!data.content || !data.content[0]) {
      return new Response(JSON.stringify({ error: "Empty response from Anthropic", detail: data }), {
        status: 500, headers: { "Content-Type": "application/json" }
      });
    }
